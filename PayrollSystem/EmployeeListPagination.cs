...
namespace PayrollSystem
{
    public class EmployeeListPagination<T> : List<T>
    {
        ...
        {
            PageIndex = pageIndex;
            TotalPages = (int)Math.Ceiling(count / (double)pageSize);//aprox prin adaos
            AddRange(items);
        }

        //enable/disable pag buttons
        public bool IsPreviousPageAvailabl => PageIndex > 1;
        public bool IsNextPageAvailabl => PageIndex < TotalPages;
